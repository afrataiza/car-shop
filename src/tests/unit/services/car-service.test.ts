...
describe("Testa a camada service da rota /cars", () => {
  ...
  describe("Testa a função create", async () => {
    ...
    it("retorna status 400, quando tenta criar um carro sem doorsQty", async () => {
      try {
        await carService.create(carMock.noDoorsCar as ICar);
      } catch (error: any) {
        expect(error.message).to.be.deep.equal(ErrorTypes.FildsMustRequired);
      }
    });

    it("retorna status 400, quando tenta criar um carro sem seatQty", async () => {
      try {
        await carService.create(carMock.noSeatsCar as ICar);
      } catch (error: any) {
        expect(error.message).to.be.deep.equal(ErrorTypes.FildsMustRequired);
      }
    });

    it("retorna status 400, quando tenta criar um carro sem model", async () => {
      try {
        await carService.create(carMock.noModelCar as ICar);
      } catch (error: any) {
        expect(error.message).to.be.deep.equal(ErrorTypes.FildsMustRequired);
      }
    });

    it("retorna status 400, quando tenta criar um carro sem year", async () => {
      try {
        await carService.create(carMock.noYearCar as ICar);
      } catch (error: any) {
        expect(error.message).to.be.deep.equal(ErrorTypes.FildsMustRequired);
      }
    });

    it("retorna status 400, quando tenta criar um carro sem color", async () => {
      try {
        await carService.create(carMock.noColorCar as ICar);
      } catch (error: any) {
        expect(error.message).to.be.deep.equal(ErrorTypes.FildsMustRequired);
      }
    });

    it("retorna status 400, quando tenta criar um carro sem buyValue", async () => {
      try {
        await carService.create(carMock.noBuyValueCar as ICar);
      } catch (error: any) {
        expect(error.message).to.be.deep.equal(ErrorTypes.FildsMustRequired);
      }
    });
